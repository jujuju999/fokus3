...
## Scope V2.5 — Auto-Planung der Heute-Aufgaben

Heute-Aufgaben **mit** Dauer bekommen automatisch einen Zeit-Slot (Spalten `scheduled_start/end`, `schedule_locked`, `planned_at`). Logik rein client-seitig: `src/lib/scheduling.ts` (pure Funktionen) + `useAutoScheduler` (einmal in `App.tsx` gemountet, konvergiert über Realtime). Regeln: First-Fit in Heute-Zuordnungs-Reihenfolge (`planned_at`), 15-Min-Puffer nach jedem Termin/zwischen Aufgaben, gelockte Slots (🔒, manuell gesetzt/bestätigt) werden **nie** automatisch bewegt — `schedule_locked=true` mit leeren Zeiten heißt „bewusst ohne Zeit". Passt nichts: Amber-Chip „Keine Zeit heute" (warnt, blockiert nie). Slots erscheinen als Sekundär-Blöcke im Wochen-Balken (gestrichelt = Auto, solide = fixiert, nie antippbar). Wachzeiten dürfen über Mitternacht gehen (Ende < Beginn); Termine bleiben auf den Kalendertag begrenzt. Abhaken/Entfernen/Morgen-Reset löscht Slots. Realtime-Kanäle brauchen **eindeutige Namen pro Hook-Instanz** (supabase-js wirft sonst bei mehrfach genutzten Hooks).
